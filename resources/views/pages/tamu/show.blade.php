                    ...
                    {{-- <div class="row">
                        <div class="col-md-6 mb-3">
                            ...
                            <h6>
                                ...
                                {{ $tamu->tanggal_kunjungan ? \Carbon\Carbon::parse($tamu->tanggal_kunjungan)->format('d F
                                Y') : '-' }}
                            </h6>
                        </div>
                                ...
                                <div class="timeline-item">
                                    ...
                                    <div class="timeline-content">
                                        <div class="alert alert-success mb-0 py-2">
                                            ...
                                            <strong>Total {{ $tamu->approvals->count() }} pegawai</strong> telah menyetujui
                                            kunjungan ini
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    <div class="d-grid gap-2">
                        ...
                        @endif
                        {{-- SECURITY - Bisa approve hanya kalau belum ada pegawai yang approve --}}
                        @if(auth()->user()->role_id == 3)
                            @if($tamu->status == 'checkin')
                                @if($tamu->approvals->count() == 0)
                                    <form action="{{ route('security.approve', $tamu->id) }}" method="POST">
                                        @csrf
                                        <button type="submit" class="btn btn-primary w-100">
                                            <i class="mdi mdi-check-circle me-1"></i>
                                            Setujui Kunjungan (Security)
                                        </button>
                                    </form>
                                @else
                                    <button type="button" class="btn btn-secondary w-100" disabled>
                                        <i class="mdi mdi-check-circle-outline me-1"></i>
                                        Sudah Disetujui Pegawai
                                    </button>
                                    <div class="alert alert-info mt-3 mb-0">
                                        <i class="mdi mdi-information-outline me-1"></i>
                                        Kunjungan ini sudah disetujui oleh pegawai:
                                        <strong>{{ implode(', ', $tamu->approver_names ?? []) }}</strong>
                                    </div>
                                @endif
                            @endif
                        @endif
...
@push('scripts')
    ...
    <script>
        ...
        $(document).ready(function () {
            $('form[action*="checkin"]').on('submit', function (e) {
                if (!confirm('Apakah Anda yakin akan check-in tamu {{ $tamu->nama }}?')) {
                    e.preventDefault();
                }
            });

            $('form[action*="approve"]').on('submit', function (e) {
                if (!confirm('Apakah Anda yakin ingin menyetujui tamu {{ $tamu->nama }}?')) {
                    e.preventDefault();
                }
            });

            $('form[action*="checkout"]').on('submit', function (e) {
                if (!confirm('Apakah Anda yakin akan check-out tamu {{ $tamu->nama }}?')) {
                    e.preventDefault();
                }
            });
        });
